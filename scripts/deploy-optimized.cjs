...
async function main() {
  ...
  const gasPriceForCalc = feeData.gasPrice || BigInt(0);
  const gasPriceGwei = Number(gasPriceForCalc) / 1e9;
  
  // Calculate costs
  const enrollCostCELO = Number(enrollReceipt?.gasUsed || 0) * Number(gasPriceForCalc) / 1e18;
  const completeCostCELO = Number(completeReceipt?.gasUsed || 0) * Number(gasPriceForCalc) / 1e18;
  const batchCostCELO = Number(batchReceipt?.gasUsed || 0) * Number(gasPriceForCalc) / 1e18;
